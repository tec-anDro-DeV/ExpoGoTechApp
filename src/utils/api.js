//LIVE new
//export const BASEURL = 'http://20.55.98.89/';

//LIVE older
export const BASEURL = 'https://urbangraffitilogin.com/';

class API {
  async getData() {
    const query = await fetch(BASEURL + 'API/v2/index.php/');
    const data = query.json();
    return data;
  }

  async valLogin(user, pass) {
    const query = await fetch(BASEURL + 'API/v1/index.php/login/user_login?username=' + user + '&password=' + pass);
    const data = query.json();
    return data;
  }

  async get_orders(user_id, token) {
    const query = await fetch(BASEURL + 'API/v1/index.php/work_orders/orders_list?user_id=' + user_id + '&token=' + token + '');
    console.log(query);
    const data = query.json();
    return data;
  }
  ...
  async get_info_new_order(user_id, token) {
    const query = await fetch(BASEURL + 'API/v1/index.php/work_orders/get_info_new_order?user_id=' + user_id + '&token=' + token + '');
    const data = query.json();
    return data;
  }
  ...
  async get_zone_new_order(user_id, token, latitude, londitude) {
    ...
    console.log(body);
    const query = await fetch(BASEURL + 'API/v1/index.php/work_orders/determinate_inside', {
      method: 'POST',
  ...
  async get_zone_new_order2(user_id, token, contract_id, zones_id) {
    ...
    const query = await fetch(BASEURL + 'API/v1/index.php/work_orders/get_zones_contract', {
      method: 'POST',
      headers: { 'Content-Type': 'multipart/form-data', otherHeader: 'foo' },
      body: body,
    });
  ...
  async new_order(user_id, token, location, address_lat, address_long, contract_id, zone_id, status, notes, surface_id, service_id, square_footage, priority) {
    ...
    console.log(body);
    const query = await fetch(BASEURL + 'API/v1/index.php/work_orders/new_order', {
      method: 'POST',
      headers: { 'Content-Type': 'multipart/form-data', otherHeader: 'foo' },
      body: body,
    });
  ...
  async get_order(user_id, token, order_id) {
    const query = await fetch(BASEURL + 'API/v1/index.php/work_orders/get_order?user_id=' + user_id + '&token=' + token + '&order_id=' + order_id);
    console.log(query);
    const data = query.json();
    return data;
  }
  ...
  async upload_img(user_id, token, order_id, file_route, correlative, type) {
    try {
      ...
      const query = await fetch(BASEURL + 'API/v1/index.php/work_orders/upload_image', {
        method: 'POST',
        headers: { 'Content-Type': 'multipart/form-data', otherHeader: 'foo' },
        body: body,
      });
  ...
  async update_order(
    ...
    console.log(body);
    const query = await fetch(BASEURL + 'API/v1/index.php/work_orders/update_order', {
      method: 'POST',
      headers: { 'Content-Type': 'multipart/form-data', otherHeader: 'foo' },
      body: body,
    });
  ...
  async new_note(user_id, token, order_id, note) {
    try {
      ...
      console.log(body);
      const query = await fetch(BASEURL + 'API/v1/index.php/work_orders/new_note', {
        method: 'POST',
        headers: { 'Content-Type': 'multipart/form-data', otherHeader: 'foo' },
        body: body,
      });
